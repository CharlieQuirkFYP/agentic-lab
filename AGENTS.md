...
The canonical development architecture is:

```text
Web voice console / benchmark dashboard
   ↓
Go API — public API, Pheme VA client, benchmark coordination
   ↓
Pheme VA (Rust) — audio, transcription, incident analysis and workflow
   ↓
Whisper/whisper.cpp + replaceable local language-model adapters
```

Pheme VA is the canonical Voice Agent implementation. It owns the portable audio pipeline, Whisper integration, model-adapter boundaries, incident workflow, sessions, confirmation, storage, and retrieval as those capabilities are implemented. Speech output uses a replaceable speech-synthesis adapter, initially at the client. Benchmark workers exercise the same Pheme VA host/API used by interactive requests. SQLite is planned for Pheme-owned incident/session storage and separately for Go-owned experiment storage; neither service accesses the other’s database directly.

The Go API remains the application-facing backend. Keep AI-heavy functionality behind integration interfaces; do not couple handlers to a model runtime. Pheme VA owns authoritative conversation state, validates AI-proposed actions, and enforces confirmation. Go delegates incident operations through its Pheme client and owns benchmark orchestration/results; do not duplicate incident workflow rules in Go. Start with an explicit state machine; LangGraph is not required for the initial workflow.

The former Python implementation has been removed. Do not reintroduce a Python runtime; the historical Python contract documentation remains only for reference. The preferred eventual target is a mobile phone, potentially an iOS device supplied by KLASS; NVIDIA Jetson is an alternative. Hardware specifications and selection are pending. Build the local Rust prototype now. Do not assume the development host layout ships unchanged inside an iOS app, or that remote inference accessed from a phone qualifies as on-device inference.

See [requirements](docs/requirements.md), [architecture](docs/architecture.md), [benchmark methodology](docs/benchmark-methodology.md), [implementation roadmap](docs/roadmap.md), and the [Pheme VA README](pheme-va/README.md). These distinguish planned work from implemented behaviour.
...
The planned real implementation is `PhemeVAIncidentAnalyzer`. Retain the mock for isolated development and tests. Keep multi-turn conversation logic inside `pheme-va/` and add corresponding Go client methods when needed, rather than forcing it into the one-shot `Analyze` contract. The Pheme VA host/API contract should define the integration; the former Python contract is historical only.
...
- Do not tightly couple the Go backend or Pheme core to a particular model, Whisper implementation, llama.cpp, or orchestration library.
...
## Rust/Pheme VA Conventions

- Keep the canonical voice-agent implementation under `pheme-va/`; the former Python implementation has been removed.

* Keep host concerns in the CLI/server/FFI crates and portable audio, transcription, and domain logic in `pheme-va/crates/core/`.
* Keep runtime integrations behind replaceable traits where multiple model backends may be evaluated.
* Run `cargo fmt --all -- --check`, `cargo clippy --workspace --all-targets -- -D warnings`, and `cargo test --workspace` from `pheme-va/`.
* Tests must use local fakes or small fixtures by default. Model-backed tests may be explicitly ignored and must take model/audio paths from environment variables.
* Do not commit model weights, recordings, or generated benchmark artifacts. Use `pheme-va/models/` for ignored local weights and document their source/checksum.
...
GitHub Actions PR CI runs separate Go backend, web frontend, and Pheme VA workflows.
...
- portable WAV/PCM normalization, mono 16 kHz conversion, speech gating, dictionary prompts, transcript guards, and timing metadata
- replaceable transcription, cleanup, and language-model adapter traits
...
Whisper/whisper.cpp is the working speech-recognition baseline discussed with KLASS. Pheme VA currently uses an optional in-process `whisper-rs` backend; exact model sizes, quantization, and runtime builds must be evaluated and recorded. A llama.cpp-compatible language-model adapter remains a replaceable implementation choice. Do not present the current model/runtime combination as a validated device configuration.

Keep transcription, language-model inference, and speech synthesis replaceable. Use persistent runtimes where appropriate rather than reloading weights each turn. Keep authoritative session state and incident tool execution in Pheme VA. Its workflow validates model proposals and controls report finalization; Go forwards operations without maintaining another session state machine.
...
The active top-level repository structure is:
...
`api/`, `web/`, `pheme-va/`, and `docs/` exist. `pheme-va/` is the canonical voice-agent implementation; the former Python runtime has been removed from the worktree. Other directories are optional future locations, not scaffolding requirements; the benchmark lifecycle currently lives in `api/internal/benchmark/`.
