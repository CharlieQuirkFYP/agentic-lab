...
* Existing benchmark storage holds experiments, not incident reports. The new `pheme-va/` workspace implements the portable audio pipeline, adapter traits, an optional in-process `whisper-rs` backend, a development HTTP host, a microphone TUI, and a mobile-oriented C ABI. Conversation state and persistent reports remain unimplemented.
